...
export const projects: ProjectDetail[] = [
  ...
  },
  {
    ...
    title: "바닐라 JS + TypeScript 기반 반응형 쇼핑몰",
    ...
    summary:
      "바닐라 JavaScript와 TypeScript를 기반으로 커머스 핵심 기능을 구현한 반응형 쇼핑몰 팀 프로젝트입니다.",

    description:
      "Nike UI/UX를 참고해 구현한 Vite 기반 멀티 페이지 쇼핑몰 프로젝트입니다. 저는 Web Components 기반 헤더·푸터 공통 컴포넌트와 상품 상세 페이지를 담당했으며, 로그인 상태별 헤더 UI 분기와 서버 기준 위시리스트 상태 동기화를 구현했습니다.",
    contributions: [
      "Web Components 기반 헤더 / 푸터 공통 컴포넌트 구현",
      "상품 상세 페이지 UI 및 기능 구현",
      "로그인 상태에 따른 공통 헤더 UI 분기 처리",
      "상품 상세 페이지의 서버 기준 위시리스트 상태 동기화",
    ],
    features: [
      "헤더와 푸터를 Web Components로 분리해 여러 페이지에서 재사용 가능한 공통 UI 구조 구현",
      "로그인 여부에 따라 회원가입 / 로그인 / 로그아웃 / 위시리스트 아이콘이 다르게 노출되도록 헤더 UI 분기 처리",
      "상품 상세 페이지에서 썸네일 선택 시 대표 이미지가 변경되도록 구현",
      "상품 상세 진입 시 서버에서 위시리스트 목록을 재조회해 새로고침 이후에도 정확한 찜 상태 유지",
      "TypeScript와 Axios 기반 API 연동을 통해 상품 데이터와 사용자 상태를 화면에 반영",
    ],
    troubleshooting: [
      {
        title: "상세 페이지 위시리스트 상태 불일치 문제 해결",
        description:
          "초기에는 상품 상세 페이지의 찜 상태를 클라이언트 내부 값에만 의존하고 있어, 새로고침 후 하트 아이콘이 실제 서버 상태와 다르게 보이는 문제가 있었습니다. 이를 해결하기 위해 페이지 진입 시 서버에서 위시리스트 목록을 다시 조회하고, 현재 상품이 포함되어 있는지 확인한 뒤 아이콘 상태를 초기화하도록 수정했습니다. 그 결과 새로고침 이후에도 서버 데이터와 화면 상태가 일치하도록 개선할 수 있었습니다.",
      },
      {
        title: "공통 헤더의 인증 상태 기반 UI 분기 구조 정리",
        description:
          "여러 페이지에서 동일한 헤더를 사용해야 했기 때문에 로그인 여부에 따라 버튼과 아이콘이 일관되게 노출되어야 했습니다. 이를 위해 헤더 컴포넌트 내부에서 accessToken 존재 여부를 기준으로 회원가입 / 로그인 / 로그아웃 / 위시리스트 아이콘 노출을 제어했습니다. 그 결과 페이지마다 별도의 인증 UI 로직을 작성하지 않고도 공통 헤더에서 인증 상태에 맞는 UI를 유지할 수 있었습니다.",
      },
    ],
    links: {
      github: "https://github.com/sang-3/vanilla-09-justCodeIt",
